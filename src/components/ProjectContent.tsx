...
export function ProjectContent() {
  ...
  const { urls, isLoading, error } = useProjectUrls(currentProject?.id);
  ...
  const handleCreateUrl = async (path: string, targets: { url: string; percentage: number }[]) => {
    ...
    try {
      await createRandomUrl(currentProject.id, path, targets);
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <URLTable urls={urls} onDelete={handleDeleteUrl} />
      )}
